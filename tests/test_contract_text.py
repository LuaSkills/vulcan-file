...
def test_skill_manifest_contract() -> None:
    ...
    assert_contains(skill_yaml, "version: 0.1.13")
    ...
    assert_contains(edit_schema, '"line_tolerance"')
    assert_contains(edit_schema, '"default": 0')
    assert_contains(edit_schema, "stale start_line/end_line")
...
def test_runtime_guardrails() -> None:
    ...
    assert_contains(edit_runtime, "compact_error_detail")
    assert_contains(edit_runtime, "DEFAULT_LINE_TOLERANCE")
    assert_contains(edit_runtime, "resolve_unique_match_range")
    assert_contains(edit_runtime, "line_tolerance")
    assert_contains(edit_runtime, "tolerant_match")
    assert_contains(edit_runtime, "matched_original_range")
    ...
    assert_contains(shared_runtime, "edited_start - 3")
...
def test_documentation_guidance() -> None:
    ...
    assert_contains(docs, "[original]")
    assert_contains(docs, "[final]")
    assert_contains(docs, "single-line decoder")
    assert_contains(docs, "line_tolerance")
    assert_contains(docs, "strict")
    assert_contains(docs, "matched_original_range")
