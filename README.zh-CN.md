...
可选的根级 `line_tolerance` 默认为 `0`，保持严格的行号范围匹配。当 AI 知道行号可能已经过期时，可以传入非负值，例如 `50`；运行时会把每个声明范围向上、向下各扩展指定的逻辑行数，并且只有当完整的、全文件唯一的 `old_content` 匹配落在扩展范围内时才接受。结果会同时报告声明范围、实际命中范围和最终范围。
...
- 宽限制匹配会显示声明范围、实际命中的原始范围和最终范围
- 节点失败时的已提交前缀（仅预览时显示为暂存前缀）、失败节点映射后的当前内容和未执行后续节点
- 内部暂存一致性失败不会写入前缀；写入失败后必须重新读取文件，因为磁盘状态可能不确定
- 面向多个节点的 diff 预览
- 多节点预览明确标注删除内容的 `[original]` 坐标，以及上下文和插入内容的 `[final]` 坐标
- 宿主 `change_set` hunk 使用最终文件上下文，同时保留原始删除内容
- 宽限制宿主 hunk 保留 `original_range`，并增加 `matched_original_range` 与 `match_mode`
- 最终 JSON 校验 detail 只返回紧凑的单行诊断
